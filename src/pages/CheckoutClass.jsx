...
const CheckoutClass = () => {
  ...
  const { class_ids = [], pet_id = null } = location.state || {};
  ...
  const getCheckoutData = async () => {
    ...
    try {
      ...
      const res = await post(`/checkout/class`, data);
      if (res.data.status) {
        const checkoutData = res.data.result;

        const orderData = checkoutData.order_details;
        setClasses(orderData.group_classes);
        setTotalPrice(orderData.total_price);
        setImageUrl(orderData.service_img_url);
        ...
        const discountData = checkoutData.discount;
  ...
  const onClickPlaceOrder = async () => {
    ...
    const data = {
      ...
      email: email,
      amount: parseFloat(totalPrice) * 100 - discountAmount * 100,
      line1: address1,
      line2: address2,
      country: countryCode,
      city: city,
  ...
  const onCompletePayment = async () => {
    ...
    const data = {
      amount: parseFloat(totalPrice) - discountAmount,
    ...
    try {
      ...
      if (resResult.status) {
        ...
        navigate(`/service/detail/${serviceId}?tab=appointments`);
      } else {
        toast.error(resResult.message);
      }
    } catch (err) {
                ...
                <div className="added-product-summary mb-20">
                  ...
                  {classes && classes.length > 0 && (
                    <div className="additional-services mt-3">
                      <h6 className="title-14 mb-2">Classes Detail</h6>
                      {classes.map((cls, idx) => (
                        <div key={idx} className="mt-2">
                          <div className="service-item d-flex justify-content-between align-items-start">
                            <p className="mb-0 text-dark">{cls.name}</p>
                            <div className="text-dark">
                              ${Number(cls.price).toFixed(2) || "0.00"}
                            </div>
                          </div>
                  ...
                  <table className="table cost-summery-table">
                    <tbody>
                      {discountAmount > 0 && (
                        ...
                        <tr>
                          <th className="text-dark pt-2 pe-2">Total</th>
                          <th className="text-dark pt-2 pe-2">
                            $
                            {(
